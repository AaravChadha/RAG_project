...
- [ ] **Production provider decision (NEW 2026-05-18).** Groq free tier blew the per-minute 8K TPM cap mid-tool-loop. Single requests now run 8-10K tokens (SYSTEM_PROMPT ~4K + TOOLS ~600 + `get_full_snapshot` payload ~2K + multi-turn history 0-3K + tool-result accumulation 1.5-3K per iteration). Choose between (a) **Groq Dev tier paid** — same model, no TPM cap, ~$15-130/month at projected volumes; (b) **Gemini Flash 2.5 primary** — calibrated 7/10 baseline today, higher TPM, requires re-validating against a wider eval; (c) Anthropic Claude Haiku 4.5 — aggressive 90% prompt-cache discount makes it cheapest on a stable-prefix workload like ours, ~$80-150/month. Real cost projection per the 2026-05-18 session: $400-800/month at 350 RMs × 30 q/day with both prompt-caching + app-level query cache. Compliance angle: ask Bajaj whether public-derivable fund data going through external APIs is OK before locking in.
- [ ] **App-level query cache (NEW 2026-05-18).** ~3-4 hours. Hash `(normalized_question, report_month)` → cached `(answer, tool_calls_json, model_name)`. Bypass the LLM on hits. Expected 30-50% hit rate. Saves ~$290-340/month at 350-RM scale. Implementation surface:
  - New `db/migrations/003_query_cache.sql` table (`cache_key TEXT PRIMARY KEY`, `question_norm`, `report_month`, `answer`, `tool_calls_json`, `model_name`, `hit_count`, `created_at`, `last_hit_at`)
  - New `retrieval/query_cache.py` module: normalize (lowercase, strip whitespace, canonicalise scheme mentions to scheme_id-keyed form via `lookup_scheme`), hash, lookup, store
  - Modify `ask()`: check cache before LLM; on miss run normally + cache if eligible; replay tool_calls_json on cache_hit log row for audit parity
