...
## Phase 1 — Vertical slice (spine) [Day 1]
...
- **1.1.6** Write minimal `README.md` with: one-paragraph project description, `python -m venv && pip install -r requirements.txt`, `cp .env.example .env`, `python db/init_db.py`, `streamlit run app/streamlit_app.py`.

**Acceptance:** `pip install -r requirements.txt` runs clean in a fresh venv. `python -c "import config; print(config.DB_PATH)"` works.

### 1.2 DB schema (with the 7 fixes)
- **1.2.1** Write `db/schema.sql` containing the original schema from `PLANNING_PROMPT.md:114-237` plus the 7 deltas:
  - **1.2.1.1** Add `schemes.scheme_uid TEXT UNIQUE NOT NULL`.
  - **1.2.1.2** Add `scheme_aliases` table.
  - **1.2.1.3** Add `fund_snapshots.revision INT DEFAULT 1`, `superseded_at TIMESTAMP NULL`. Replace `UNIQUE(scheme_id, report_month)` with `UNIQUE(scheme_id, report_month, revision)`.
  - **1.2.1.4** Add `fund_snapshots.parser_version`, `parse_errors_json`, `pdf_sha256`.
  - **1.2.1.5** Drop `sector_weights_json` column. Add `sector_weights(snapshot_id, sector, weight_pct)` table.
  - **1.2.1.6** Drop `monthly_returns_json`, `fy_returns_json`, `cy_returns_json`. Add `periodic_returns(snapshot_id, period_type, period_label, return_pct)` table.
  - **1.2.1.7** Beef up `query_log` with: `tool_calls_json`, `model_name`, `model_version`, `latency_ms`, `tokens_in`, `tokens_out`, `refusal_reason`.
  - **1.2.1.8** Add `idx_hold_security_month` index on `holdings(security_name, report_month)`.
  - **1.2.1.9** Add `schema_version` table seeded with version 1.
- **1.2.2** Write `db/init_db.py`: idempotent — drops & recreates the DB if `--force`, else creates tables only if absent. Reads `db/schema.sql` and executes.
- **1.2.3** Write `db/seed_schemes.py` (or fold into `init_db.py`): reads `schemes_master.csv` and inserts into `schemes` table. Generates `scheme_uid` as a deterministic slug (e.g., `slugify(amc + scheme_name)`).
- **1.2.4** Add `db/migrations/` folder with `001_initial.sql` containing the same schema (for future `pgloader` migration to Postgres).
...
### 1.3 Stub parser — 5 fields from one PDF
- **1.3.1** Define `Snapshot` dataclass in `ingest/models.py` with all the columns of `fund_snapshots`. Default to `None` for everything; populate as parsers run.
- **1.3.2** Define `ParseError` dataclass with `section`, `error`, `traceback`.
- **1.3.3** Write `ingest/parse_finalyca.py` with a single function `parse_pdf_minimal(path) -> Snapshot` that extracts ONLY:
  - **1.3.3.1** `scheme_name` (from header — use PyMuPDF text extraction)
  - **1.3.3.2** `as_of_date` (from header — parse "04 May 2026" format)
  - **1.3.3.3** `expense_ratio` (from header block)
  - **1.3.3.4** `return_1y` (from Trailing Returns table — use pdfplumber)
  - **1.3.3.5** `fund_aum_cr` (from header block)
- **1.3.4** Write `ingest/ingest_one.py`: CLI that takes a PDF path, calls `parse_pdf_minimal`, looks up `scheme_id` by name, inserts into `fund_snapshots` (with `report_month='2026-05'`, `parser_version='0.1-spine'`).
...
### 1.5 Hardcoded SQL chatbot (the spine)
- **1.5.1** Write `retrieval/db_query.py` with `query_db(sql, params=()) -> list[dict]`. Uses read-only SQLite connection (`sqlite3.connect("file:bajaj_mf.db?mode=ro", uri=True)`). Refuses any SQL containing `INSERT|UPDATE|DELETE|DROP|ALTER|CREATE`.
- **1.5.2** Write `app/chatbot.py` exposing `ask(question: str) -> str`. Day-1 implementation is naive: a `dict` mapping a few hardcoded question patterns to SQL. ONE entry: `"expense ratio of <scheme>"` → `SELECT expense_ratio FROM fund_snapshots WHERE scheme_id IN (SELECT scheme_id FROM schemes WHERE scheme_name LIKE ? LIMIT 1)`.
- **1.5.3** `ask()` uses LLMClient ONLY to format the final answer ("The expense ratio of X as on Y is Z%"). It does NOT yet do tool-use — that's Phase 5.
- **1.5.4** `ask()` writes a row to `query_log` with the question, SQL, final answer, and model name.
- **1.5.5** Citation format: every answer ends with `\n\nSource: <scheme_name>, as on <as_of_date>`.
...
## Phase 2 — Eval-driven spec [Day 2]
...
- **2.2.3** Write 3 refusal questions (one per refusal category) with `"must_refuse": true` and `"expected_refusal_reason": "unknown_scheme" | "no_data" | "out_of_scope"`. Refusals do NOT include the verification footer (a refusal isn't research output). Total refusals: 3. **Grand total: ~39 questions.**
- **2.2.4** Hand-verify expected answers against the 3 sample PDFs (look up the actual numbers; don't guess). For every non-refusal question, include the literal substring `"verify against your own"` in `expected_answer_contains` as a regression check on the universal-footer behavior.

### 2.3 Eval harness
- **2.3.1** Write `tests/test_chatbot.py` with one parametrized test per golden question.
- **2.3.2** Test passes if (a) `must_refuse` is false AND `expected_answer_contains` substrings ALL appear in answer, OR (b) `must_refuse` is true AND `refusal_reason` matches.
- **2.3.3** Output a results report: `<n> passed, <n> failed, <n> with wrong SQL`. Save to `tests/last_eval_report.json`.

**Acceptance:** `pytest tests/test_chatbot.py` runs (will mostly fail today — that's fine, the spec exists).

---

## Phase 3 — Parser widening [Day 3-5]
...
### 3.1 Establish per-section structure
- **3.1.1** Refactor `parse_finalyca.py` to the dispatch pattern: `SECTION_PARSERS = [parse_header, parse_returns, parse_risk_metrics, ...]`. Each function takes `(doc, pl, snap, errors)`.
- **3.1.2** Wrap every section call in `try/except` — exceptions append to `errors`, never abort the whole parse. Exception: `parse_header_required` aborts if scheme_name or as_of_date is missing (no PK = no insert).
- **3.1.3** Define `EXPECTED_SECTIONS_BY_FUND_TYPE` map. Equity funds expect equity sectors; debt funds don't. Use this to distinguish "section missing because fund type doesn't have it" from "section missing because parser broke."

### 3.2 Section parsers — one subtask each
- **3.2.1** `parse_header` — Benchmark, Inception Date, Min Investment, Expense Ratio, Exit Load, Fund AUM, Age, As-Of date, Overview paragraph.
- **3.2.2** `parse_fund_managers` → `fund_managers_json`.
- **3.2.3** `parse_trailing_returns` — fund + benchmark, all 9 periods.
- **3.2.4** `parse_risk_metrics` — 1Y and 3Y blocks, all 10 metrics each.
- **3.2.5** `parse_sector_weights` → rows in `sector_weights` table (NOT a JSON column, per schema fix 1.2.1.5).
- **3.2.6** `parse_top_holdings` — top 10 (separate from full holdings table).
- **3.2.7** `parse_portfolio_characteristics` — Total Securities, Avg/Median Mkt Cap, P/E, P/B, Div Yield, Modified Duration.
- **3.2.8** `parse_composition` → `composition_json`.
- **3.2.9** `parse_drawdown` — pct, duration, peak/valley/recovery dates.
- **3.2.10** `parse_risk_rating` → `risk_rating_json`.
- **3.2.11** `parse_market_cap_composition` — large/mid/small.
- **3.2.12** `parse_investment_style` → `investment_style_json`.
- **3.2.13** `parse_periodic_returns` — monthly + FY + CY → rows in `periodic_returns` table.
- **3.2.14** `parse_holdings_full` — full multi-page holdings table → rows in `holdings` table. Concatenate across pages BEFORE parsing.
- **3.2.15** Cross-cutting: NA → NULL, "0E-9" → 0.0, date normalization to ISO.

### 3.3 Invariants module
- **3.3.1** Write `ingest/invariants.py` with named check functions:
...
- **3.3.2** Run all invariants after parsing; failures append to `parse_errors_json` but do NOT block insert.

### 3.4 Golden samples
- **3.4.1** Hand-extract ~20 critical fields per sample PDF into `tests/golden/canara_robeco_multi_cap.json`, `tests/golden/absl_arbitrage.json`, `tests/golden/dsp_multi_asset.json`. ~2 hours one-time work.
- **3.4.2** Write `tests/test_parser.py`: parse each sample PDF, assert each golden field matches.

### 3.5 Diff regression baseline
- **3.5.1** After parsers are stable, snapshot all parsed outputs to `tests/snapshots/` as JSON. Future runs `deepdiff` against this — any unexpected delta gets flagged.
